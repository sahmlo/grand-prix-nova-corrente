    ...
    // Configurações padrão do sistema - define cores, fontes e textos iniciais
    const defaultConfig = {
      app_title: "Nova Corrente",
      app_subtitle: "Previsibilidade de Demanda", 
      welcome_message: "Bem-vindo ao painel de controle",
      background_color: "#0a0e1a",      // Cor de fundo principal
      surface_color: "#1a1f2e",         // Cor dos cards e superfícies
      text_color: "#e4e7eb",            // Cor do texto principal
      primary_action_color: "#3b82f6",  // Cor dos botões principais
      secondary_action_color: "#64748b", // Cor dos elementos secundários
      font_family: "Inter",             // Fonte padrão
      font_size: 14                     // Tamanho base da fonte
    };

    // ===== VARIÁVEIS GLOBAIS =====
    let allData = [];           // Array que armazena todos os dados do sistema
    let isLoading = false;      // Flag para controlar estados de carregamento
    let filteredData = [];      // Array para dados filtrados
    let currentEditingId = null; // ID do item sendo editado
    let currentUser = null;     // Usuário logado atual
    let isLoggedIn = false;     // Status de login

    // ===== MANIPULADOR DE DADOS =====
    // Função chamada automaticamente quando os dados são alterados no backend
    const dataHandler = {
      onDataChanged(data) {
        ...
        // Atualiza todas as interfaces do sistema
        updateDashboard();
        updateItemsTable();
        updateServicesTable();
        updateLossesTable();
    ...
    function initializeAuth() {
      ...
      if (savedSession && rememberMe) {
        try {
          ...
          // Sessão válida por 7 dias se "lembrar-me" estiver marcado
    ...
    function setupScrollAnimations() {
      ...
      // Observer para detectar elementos na viewport
    ...
    async function initializeApp() {
      ...
      // Inicializa SDK de dados para persistência
      ...
      // Inicializa SDK de elementos para customização
      await window.elementSdk.init({
        defaultConfig,
        
        // Função chamada quando configurações são alteradas
        ...
        // Define capacidades de customização
        ...
        // Mapeia valores para o painel de edição
    ...
    // Configura eventos de clique para todos os itens de navegação
    ...
    function navigateToPage(pageName) {
      // Remove classe 'active' de todos os itens de navegação e páginas
      document.querySelectorAll('.nav-item').forEach(i => i.classList.remove('active'));
      document.querySelectorAll('.page').forEach(p => p.classList.remove('active'));
      
      // Adiciona classe 'active' ao item e página selecionados
      ...
      // Atualiza título da página no navbar
      document.getElementById('pageTitle').textContent = titles[pageName] || 'Dashboard';
      
      // Salva página atual no localStorage para persistência
    ...
    // ===== FORMULÁRIO DE ITENS =====
    // Configura evento de submissão do formulário de itens
    ...
    // Configura evento de submissão do formulário de serviços
    ...
    // Botão para limpar formulário de serviços
    ...
    // ===== SISTEMA DE PERDAS =====
    // Abre modal para registrar nova perda
    ...
    // Submissão do formulário de perdas
    ...
    function updateDashboard() {
      ...
      // Atualiza status do card de perdas baseado na quantidade
    ...
    function updateItemsTable() {
      ...
      if (document.getElementById('itemSearch')) {
        ...
        // Fallback para quando os filtros ainda não existem
    ...
    function updateServicesTable() {
      ...
      // Ordena serviços por data de criação (mais recentes primeiro)
    ...
    function updateAnalytics() {
      ...
      // Atualiza indicadores de projeção (simulados)
    ...
    function generateChatResponse(message) {
      ...
      // Respostas sobre análises e relatórios
...
Use o menu lateral para navegar entre as seções!`;
    ...
    function bulkDeleteItems() {
      ...
      // Cria modal de confirmação para exclusão em lote
      const confirmModal = document.createElement('div');
      confirmModal.className = 'modal-overlay active';
      confirmModal.innerHTML = `
        <div class="modal">
    ...
    function clearAllData() {
      ...
      // Cria modal de confirmação crítica
      const confirmModal = document.createElement('div');
      confirmModal.className = 'modal-overlay active';
      confirmModal.innerHTML = `
        <div class="modal">
      ...
      // Habilita botão apenas quando "CONFIRMAR" for digitado
    ...
    document.addEventListener('keydown', (e) => {
      // Apenas processa se não estiver em um campo de input
    ...
    function setupMobileFeatures() {
      ...
      // Detecta cliques fora da sidebar no mobile
      ...
      // Adiciona botão de menu mobile (se necessário)
      if (window.innerWidth <= 768) {
    ...
    async function startApplication() {
      ...
      try {
        // Inicializa sistema de autenticação primeiro
        initializeAuth();
        
        // Configura formulários de login e senha
        setupLoginForm();
        ...
        // Configura recursos mobile
        setupMobileFeatures();
        
        // Configura animações de scroll
        setupScrollAnimations();
        
        // Inicializa SDKs independente do login (para funcionar quando usuário logar)
        // Os SDKs ficam prontos mas só são usados após login
        if (window.dataSdk && window.elementSdk) {
          await initializeApp();
        }
        
        // Se usuário está logado, exibe mensagem de boas-vindas
        if (isLoggedIn) {
          setTimeout(() => {
